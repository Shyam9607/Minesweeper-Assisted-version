...
class DPSolver:
    ...
    def get_move(self, board, is_hint=False):
        ...
        # DIVIDE (Same as D&C, needed to make DP mathematically possible)
        self.clusters = self.find_clusters(frontier, board)
        
        all_safe_reveals = []
        all_safe_flags = []

        # CONQUER: Use Dynamic Programming on each cluster
    ...
    def dp_solve_cluster(self, cluster, board):
        ...
        """
        # 1. Gather all hidden variables for this cluster
        hidden_set = set()
        for cell in cluster:
        ...
        # 2. Setup Initial State Constraints
        # Calculate how many more mines each numbered cell requires
        initial_needs = []
        for cell in cluster:
        ...
        memo = {}
        mine_counts = {h: 0 for h in hidden_list} # Tracks in how many valid configs this is a mine

        # 3. Recursive DP Function with Memoization
        def dp(index, current_needs):
            # STATE: Current hidden cell index, and the tuple of remaining mine needs
            state = (index, tuple(current_needs))
            
            # MEMOIZATION: If we've seen this exact state before, return cached result
            if state in memo:
                return memo[state]

            # BASE CASE: Processed all hidden cells
            if index == len(hidden_list):
                # If all constraints are perfectly met (0 needs left)
            ...
            # --- BRANCH 1: Assume h_cell is SAFE ---
            # Needs do not change.
            ways_if_safe = dp(index + 1, current_needs)
            total_valid_configs += ways_if_safe

            # --- BRANCH 2: Assume h_cell is a MINE ---
            # Decrease the 'needs' of all adjacent numbered cells
            ...
            for i, constraint_cell in enumerate(cluster):
                if h_cell in constraint_cell.neighbors:
                    new_needs[i] -= 1
                    if new_needs[i] < 0: # Invalid: Assigned too many mines
            ...
            if valid_mine_placement:
                ...
                # If placing a mine here resulted in valid realities, log them
                if ways_if_mine > 0:
        ...
        # 5. Interpret DP Results
        if total_configs > 0:
            for h in hidden_list:
                # If it was a mine in EVERY valid configuration -> 100% Mine
                if mine_counts[h] == total_configs:
                    flag_moves.append(h)
                # If it was a mine in ZERO valid configurations -> 100% Safe
